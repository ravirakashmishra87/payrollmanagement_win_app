...
{
    public partial class frmUserSalaryList : BaseForm
    {


        #region CONSTRUCTOR
        public frmUserSalaryList()
        {
            InitializeComponent();
            FillGrid();
        }
                ...
                if (userdetailslist != null)
                {
                    if (userdetailslist.Count > 0)
                    {
                        ...
                        foreach (var users in userdetailslist)
                        {
                            ...
                            DGUsermaster.Rows[iRowindex].Cells["FNAME"].Value =$"{ users.FNAME} { users.LNAME}";
                            DGUsermaster.Rows[iRowindex].Cells["EMPREFNO"].Value = users.REFNO;
        ...
        private void btnNew_Click(object sender, EventArgs e)
        {
            try
            {
                if (DGUsermaster.CurrentCell != null)
                {
                    ...
                    if (IID > 0)
                    {
                        frmSalaryDetails objsalaryDetails = new frmSalaryDetails();
                        //objsalaryDetails.LOGINID = LOGINID;
                        objsalaryDetails.LOGGEDINUSERID = LOGGEDINUSERID;
                        objsalaryDetails.EMPLOYEEIID = IID;
        ...
        private void GetMasterDetails()
        {
            try
            {
                ...
                if (DGUsermaster.CurrentCell != null)
                {
                    int iRowIndex = DGUsermaster.CurrentCell.RowIndex;
                    int IID = Convert.ToInt32(DGUsermaster.Rows[iRowIndex].Cells[0].Value.ToString());
                    string employeecode = Convert.ToString(DGUsermaster.Rows[iRowIndex].Cells[2].Value);
                    if (IID > 0)
                    {
                        frmSalaryDetails objsalaryDetails = new frmSalaryDetails();                       
                        objsalaryDetails.LOGGEDINUSERID = LOGGEDINUSERID;
            ...
            try
            {
                ...
                if (DGUsermaster.CurrentCell != null)
                {
                    ...
                    if (MessageBox.Show("Really want to delete this record?", "Employee Management", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        ...
                        bool isDeleted = employeesalarylist.Remove(employeesalarylist.FirstOrDefault(u => u.EMPLOYEEIID == employeeid));
